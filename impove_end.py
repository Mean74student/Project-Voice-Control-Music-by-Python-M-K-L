...
def toggle_play_pause():
    ...
    if is_playing:
        ...
        # If no song is currently playing, attempt to play the selected song
        play_selected_song()
...
def update_time_label():
    ...
    if is_playing and not is_paused:
        if current_time < song_duration:
            time_label.config(
                text=f"Time: {time_format(current_time)} / {time_format(song_duration)}"
            )
            current_time += 1
...
time_label = tk.Label(app, text="Time: 00:00 / 00:00", font=("Helvetica", 14), bg="#2E2E2E", fg="#FFFFFF")
